...
uint32_t finalize_cbs(
    ...
    return tt::align(base_offset + total_cb_size, hal.get_alignment(HalMemType::L1));
}
...
void generate_runtime_args_cmds(
    ...
    while (num_packed_cmds_in_seq != 0) {
        ...
        uint32_t cmd_sequence_sizeB = tt::align(sizeof(CQPrefetchCmd) + rt_payload_sizeB, pcie_alignment);
...
uint32_t insert_write_packed_payloads(
    ...
    while (rem_num_sub_cmds != 0) {
        ...
        cmd_payload_sizeB += tt::align(sizeof(CQPrefetchCmd) + packed_cmd_payloads.back().second, pcie_alignment);
...
void assemble_device_commands(
    ...
    cmd_sequence_sizeB += tt::align(sizeof(CQPrefetchCmd) + sizeof(CQDispatchCmd), pcie_alignment);
...
void reset_config_buf_mgrs_and_expected_workers(
    std::array<WorkerConfigBufferMgr, dispatch_constants::DISPATCH_MESSAGE_ENTRIES>& config_buffer_mgrs,
...
void reset_worker_dispatch_state_on_device(
    ...
    uint32_t cmd_sequence_sizeB =
        reset_launch_msg_state * device->dispatch_s_enabled() *
            hal.get_alignment(
        ...
        (hal.get_alignment(
         ...
         device->distributed_dispatcher() * hal.get_alignment(HalMemType::HOST)) *
            num_sub_devices;
...
void set_go_signal_noc_data_on_dispatch(
    ...
    DispatcherSelect dispatcher_for_go_signal =
        device->dispatch_s_enabled() ? DispatcherSelect::DISPATCH_SLAVE : DispatcherSelect::DISPATCH_MASTER;
