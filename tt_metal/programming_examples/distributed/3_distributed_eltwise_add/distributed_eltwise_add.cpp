...
int main(int argc, char** argv) {
    ...
    EnqueueWriteMeshBuffer(cq, a, a_data);
    EnqueueWriteMeshBuffer(cq, b, b_data);
    ...
    auto target_devices = LogicalDeviceRange{
        CoreCoord{0, 0} /* start_coord */, CoreCoord{mesh_device->num_cols() - 1, mesh_device->num_rows() - 1}
        /* end_coord */
    };

    AddProgramToMeshWorkload(mesh_workload, program, target_devices);
    EnqueueMeshWorkload(cq, mesh_workload, false /* blocking */);

    // Read back results
    std::vector<uint32_t> result_data(distributed_buffer_size_bytes / sizeof(uint32_t));
    ...
    std::cout << "Total values: " << result_data.size() << "\n";
    for (int i = 0; i < 32; i++) {
        bool value_is_close = is_close(c_bf16[i].to_float(), golden_bf16[i].to_float());
        std::string pass_or_fail = value_is_close ? "PASS" : "FAIL";
        std::cout << pass_or_fail << "  " << a_bf16[i].to_float() << " + " << b_bf16[i].to_float() << " = "
                  << c_bf16[i].to_float() << " (golden: " << golden_bf16[i].to_float() << ")\n";
    }
