...
DispatchCoreConfig MeshDevice::get_dispatch_core_config() {
    return validate_and_get_reference_value(
        scoped_devices_->get_devices(), [](const auto& device) { return device->get_dispatch_core_config(); });
}

const std::vector<CoreCoord>& MeshDevice::get_logical_storage_cores() {
    validate_and_get_reference_value(
        scoped_devices_->get_devices(), [](const auto& device) { return device->get_logical_storage_cores(); });
    return reference_device()->get_logical_storage_cores();
}

const std::vector<CoreCoord>& MeshDevice::get_logical_dispatch_cores() {
    validate_and_get_reference_value(
        scoped_devices_->get_devices(), [](const auto& device) { return device->get_logical_dispatch_cores(); });
    return reference_device()->get_logical_dispatch_cores();
}
